...
static MenuState s_mode           = WAITING_DCS;
...
// SP at any point: calls beginConnect(true) and resets the 65-second window.
// LP at any point: sets s_mode=SETTINGS and returns false.
// Returns true when connected, false if user navigated to Settings.
static bool connectWifi() {
  ...
  while (true) {
    ...
    if (!timedOut && millis() >= deadline) timedOut = true;

    if (Encoder::shortPressed()) {
      WifiMgr::beginConnect(true);
      timedOut = false;
      deadline = millis() + 65000UL;
    }
...
void setup() {
...
#ifdef DEV_BUILD
  {
    ...
    static auto injectFn   = [](const char* verb, const char* rest) -> bool {
      if (strcmp(verb, "enc") != 0) return false;
  ...
  // USB settle — PMU shared between USB-OTG and WiFi; wait for OTG enumeration (~3 s).
  UI::showStarting();
  {
...
void loop() {
  ...
  // RWR MISSILE LAUNCH — 200 ms debounce, 100 ms flash
  ...
  if (s_mode == WAITING_DCS) {
    ...
    if (Encoder::longPressed()) { s_mode = SETTINGS; UI::flashScreen(); }
  ...
  // Macro menu auto-return: 15 s idle while DCS is streaming
  if (s_mode == MACRO_MENU && DcsBios::isConnected() &&
      millis() - s_macroLastInput > 15000UL)
    s_mode = AIRCRAFT_STATUS;

  // Reset sleep timer when leaving OTA flow
  {
    static MenuState s_prevMode = WAITING_DCS;
  ...
  if (!s_oledSleeping && millis() - s_lastOled > 200) {
    ...
    switch (s_mode) {
      ...
      case MOUSE_CALIBRATE_X:
        UI::showMouseCalibrate(0, s_calibX, kCalibLabelX[s_calibIdx]); break;
      ...
      case FIRMWARE_CONFIRM: {
        ...
        char availVer[10];
        snprintf(availVer, sizeof(availVer), "v%d.%02d",
                 s_otaResult.versionInt / 100, s_otaResult.versionInt % 100);
        UI::showFirmwareConfirm(curVer, availVer);
        break;
      }
      case FIRMWARE_UPDATING:  break;
